...
public class Processor extends JPanel {
    ...
    public ImageIcon splitRGB(String path, String channel) throws IOException {
     ...
     * @throws IOException
     */
    public ImageIcon convolution3(String path, int[] matrix) throws IOException {
        if (!is8bitgray && !isFrequency) {
            ImagePlus ip = new ImagePlus("image", ImageIO.read(new File(path)));
     ...
     * @throws IOException
     */
    public ImageIcon binarize(String path, int trigger) throws IOException {
        if (!is8bitgray || !isFrequency) {
            is8bitgray = true;
    ...
    public ImageIcon doDFT(String path) throws IOException {
        if (!isFrequency) {
    ...
    //TODO Problème ici
    public ImageIcon doIDFT(String path, int w, int h) throws IOException {
        if (isFrequency) {
...
//    public ImageIcon contrast(String path, String how) throws IOException {
//        if (!is8bitgray || !isFrequency) {
//
...
//    }
    ...
    public ImageIcon posterize(String path, int level) throws IOException {
        if (!is8bitgray || !isFrequency) {
            BufferedImage bi = ImageIO.read(new File(path));
